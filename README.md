...
### 6. 各サイトにログインしてセッション保存（**アプリ起動前に必須**）

> ⚠️ **アプリは起動時にログインセッションの存在を確認します。`enabled_sites` に含まれるサイトのセッションが 1 つでも未保存だとエラーダイアログを表示して即終了します。** 必ず先に下記のログインを済ませてください。

使いたいサイトだけでOKです（`config.json` の `enabled_sites` から外したサイトはチェックされません）：
...
| 症状 | 対処 |
|------|------|
| 起動時に「ログインセッションがありません」エラー | 表示されたサイトの `python program/<site>_login.py` を実行してからアプリ再起動 |
