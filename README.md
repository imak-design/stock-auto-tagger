...
>
> 📝 **[変更履歴 (CHANGELOG.md)](./CHANGELOG.md)** — アップデート内容・各サイトの仕様変更対応はこちら
>
...
8. **ベクター(EPS)対応**: Adobe / Shutterstock へは EPS をそのまま、Pixta へはツールが作った ZIP（EPS＋見本画像）をアップロード
...
| Pixta | JPG/PNG | MP4/MOV | ZIP（ツールが作る） |
...
> **注意**: macOS Tahoe (26) で macOS 付属の Python 3.9 を使用すると tkinter が起動しない問題を確認しています。上記の `brew install python` で Homebrew 版をインストールすることを推奨します。
...
```bash
# Homebrew の Python で venv を作成（推奨）
python3 -m venv venv
...
| `Vector/` | ベクター素材（EPS ＋ 見本画像 PNG） | EPS ファイルをアップロードする人 |
...
- Vector フォルダ内は素材ごとにサブフォルダを作ります。PIXTA 用の ZIP はツールが作ります → 下の「Vector フォルダの入れ方」

#### Vector フォルダの入れ方

素材ごとにサブフォルダを作り、**そのフォルダ名と同じ名前**で EPS と PNG を入れます。

```
input/Vector/260405_crown_vector/
    ├── 260405_crown_vector.eps   ← Adobe / Shutterstock はこれをそのまま送ります
    └── 260405_crown_vector.png   ← 解析用の見本画像。PIXTA の ZIP にも入ります
        （背景に色や白を敷いた素材は、同じ名前の 260405_crown_vector.jpg も置くと
          PIXTA へ「ベクター＋JPG」として登録されます。無くても上がります）
```

**EPS は Illustrator 10 形式で保存してください。** 3サイトの受付形式の最大公約数です
（Shutterstock は EPS 8 か 10 のみ、PIXTA は Illustrator 8.0〜CS2 互換のみ）。
EPS 8 はメタデータを保持しないので選ばないでください（PIXTA はファイルに埋め込んだタイトル・タグしか読みません）。

**PIXTA 用の ZIP はツールが作ります。ご自身で作る必要はありません。**
PIXTA だけはベクターを「EPS ＋ 見本画像 1 枚」の ZIP で受け取ります。ツールは工程1で EPS にタイトル・タグを
埋め込み、アップロードの直前にその EPS と見本画像で ZIP を作ります。中身は PIXTA 公式
「ベクター・PNG素材ガイドライン」（ https://pixta.jp/guide/?p=7136 ）の表のとおりです。

| 素材 | ZIP の中身 |
|---|---|
| **背景が透過**（PNG に透明な部分がある） | EPS + PNG（販売用の JPEG は PIXTA が PNG から自動で作ります） |
| **背景に色や白を敷いている** | EPS + JPG（同じ名前の JPG を置いたとき。無ければ EPS + PNG） |

- 見本画像は、**その EPS を開いて実寸で書き出したもの**にしてください。PIXTA は ZIP の中の EPS と見本画像の見た目が違うとリジェクトします
- **ZIP をご自身で置く必要はありません。置いてあってもツールが作り直します。** 自分で作った ZIP は中の EPS にタイトル・タグが入らず、PIXTA で登録が通りません
- 見本画像を 2 枚（PNG と JPEG）入れた ZIP は PIXTA が「複数のJPEGまたはPNGが含まれています」と拒否します。ツールは必ず 1 枚だけ入れます
- 工程1（解析）を通していない素材は PIXTA へ上がりません（ログに出ます）。工程2だけを後から実行するときも、工程1の結果から ZIP を作ります
...
### 6. 各サイトにログインしてセッション保存（**最低 1 サイトは必須**）

> ⚠️ **アプリは起動時に `enabled_sites` のセッションを確認します。**
> - **1 つも保存されていない** → エラーダイアログで即終了
> - **一部のサイトだけ保存済み** → 警告ダイアログで通知し、未保存サイトはアップロード時に自動スキップして起動継続
> - **すべて保存済み** → 通常起動

使いたいサイトだけログインすれば OK です（`config.json` の `enabled_sites` から外したサイトはチェックされません）：
...
ブラウザが開くので手動でログイン → セッションが自動保存されます。セッションが切れた場合も同じコマンドを再実行してください。

> **なぜ CLI 専用？**: Pixta の reCAPTCHA が Playwright 起動のブラウザを検知して通過不可になるため、ログインは独立プロセス（CLI）で完結させる仕様です。GUI 内からのログインフローは提供していません。

> 保存先は前述の「機密ファイルの保存場所まとめ」を参照。
...
| **工程2** Adobe | ブラウザアップロード → CSV適用 → 審査提出（※人間確認画面が出た場合は手動対応。下記参照） |
...
Adobe Stock は審査提出の直前に、人間かどうかを確認する画面（猫などの画像を選ぶ／画像を表す単語を入力する）を**ランダムに表示する**ことがあります。プログラムでは突破できないため、次の動作になります。

- ファイルの**アップロードは完了**しますが、**審査提出は完了しません**
- **最前面のポップアップで通知**します（自動実行中に別の作業をしていても気づけるように）
- **ブラウザは開いたまま**にするので、画面の指示に従って手動で審査提出を完了してください
- 全工程を実行していた場合は、Shutterstock・Pixta の処理とファイル移動は通常どおり続行し、最後にまとめて通知します
...
│   ├── AI/                   ← AI生成素材（同梱済み）
│   ├── Photo/                ← 写真素材（同梱済み）
│   ├── Vector/               ← ベクター素材（EPS + 見本画像PNG。PIXTA用ZIPはツールが作る）
...
| JPG | 対象 | 対象 | 対象 |
| 動画 | 対象 | 対象 | 対象 |
| EPS | 対象 | 対象 | ZIP（ツールが作る。透過=EPS+PNG / 不透過=EPS+JPG。見本画像は1枚だけ） |
...
| 症状 | 対処 |
|------|------|
| 起動時に「ログインセッションがありません」エラー | 有効サイトのセッションが 1 つも無い状態。表示された `python program/<site>_login.py` のいずれかを実行してからアプリ再起動 |
| 起動時に「一部サイトのログインセッションが未保存」警告 | 未保存サイトはアップロードでスキップされます。利用したい場合は表示コマンドでログイン |
...
- **Python 3.10+**（Mac は Homebrew 版を推奨）/ Tkinter（GUI）
