...
> **無料枠の目安（Gemini 2.5 Flash）**: 無料 API キーは **1日25リクエスト**（1分5リクエスト）の上限があります。画像1枚 = 1リクエストのため：
> - **工程1のみ**（タグ生成）: 1日 **約20枚**
> - **工程0から実行**（リネーム + タグ生成）: リネームにも1リクエスト使うため、1日 **約10枚**
>
...
> **API キーの取り扱いについて**
> - API キーは必ず `.env` ファイルに書いてください。`.env` はあなたのパソコンの中だけに保存され、外部に公開されることはありません
> - `config.json` など他のファイルには API キーを書かないでください
> - Claude などの AI チャットに API キーを貼り付けると会話ログに残る可能性があります。キーの設定は必ずご自身でテキストエディタから直接行ってください

#### .env ファイルの編集方法

`.env` はドットから始まる隠しファイルのため、通常のファイルマネージャーには表示されません。
ターミナル（Mac）またはコマンドプロンプト（Windows）から編集してください。

**Mac:**
```bash
open -e ~/Desktop/stock-auto-tagger/.env
```
テキストエディット（TextEdit）で開きます。

**Windows:**
```bash
notepad %USERPROFILE%\Desktop\stock-auto-tagger\.env
```
メモ帳で開きます。
...
- **Pixta**: ファイルアップロード → AIチェック → 全選択まで完了し、「選択した作品を登録」ボタンの手前で停止（confirmページには遷移しません）

停止後はブラウザが開いたままになるので、**タイトルやタグを手動で確認・修正してから審査申請ボタンを押す** ことができます。AI が生成したメタデータを確認・調整したい場合に便利です。

確認が終わったら **ブラウザを閉じると自動的に次のサイトの処理に進みます**（工程0の一気通貫実行時）。

テストモード時は工程0のファイル移動もスキップされるため、同じ素材で繰り返しテストできます。
