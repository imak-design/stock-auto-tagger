...
> **macOS Tahoe (26) 以降の場合**: macOS 付属の Python 3.9 では tkinter が正常に動作しません。必ず Homebrew の Python（3.10以上）を使用してください。`brew install python` でインストールされます。
...
```bash
# Homebrew の Python で venv を作成（macOS 付属の Python は使わないこと）
python3 -m venv venv
...
- **Python 3.10+**（Homebrew 推奨。macOS 付属の 3.9 は非対応）/ Tkinter（GUI）
